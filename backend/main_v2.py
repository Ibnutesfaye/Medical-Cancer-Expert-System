...
from routes.encrypted_inference import router as encrypted_router
...
async def lifespan(app: FastAPI):
    ...
    with Session(engine) as db:
        if config.admin_password and not get_user_by_username(db, config.admin_username):
            ...
            print(f"✅ Admin user '{config.admin_username}' seeded")
        elif not config.admin_password:
            print("ℹ️  ADMIN_PASSWORD is unset; default admin seeding skipped")
    ...
    def _build_index():
        try:
            ...
            print(f"⚠️  Dataset index build error: {e}")
...
# ── App ───────────────────────────────────────────────────────────────────────
app = FastAPI(
    title="Medical Cancer Expert System API",
...
# Audit logging middleware — records security-relevant requests to DB
try:
    from core.audit import AuditMiddleware
    app.add_middleware(AuditMiddleware)
    print("✅ Audit middleware registered")
except Exception as e:
    print(f"⚠️  Audit middleware skipped: {e}")
...
app.include_router(encrypted_router)   # encrypted inference + Grad-CAM + quick benchmark
app.include_router(benchmark_router)   # benchmark save/list/compare/stats
app.include_router(doctor_router)      # doctor dashboard + notes
...
def metrics():
    ...
    Returns prometheus_client format if installed, else RAG search metrics.
    """
    try:
...
if __name__ == "__main__":
    ...
    uvicorn.run("main_v2:app", host="0.0.0.0", port=8000, reload=False)  # nosec B104
