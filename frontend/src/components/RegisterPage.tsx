...
export default function RegisterPage({ onRegistered, onBack }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.message === 'Failed to fetch' || err.name === 'TypeError') {
        setError('Backend is not running. Run: venv\\Scripts\\python.exe main.py')
      } else {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#0f0f0f] flex items-center justify-center p-4 transition-colors duration-300"
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 dark:text-[#666] text-sm mt-1">Create your account</p>
        </div>
        ...
        <div className="bg-white dark:bg-[#161616] border border-gray-200 dark:border-[#2a2a2a] rounded-2xl p-6 shadow-2xl transition-all duration-300">
          <h2 className="text-base font-semibold text-gray-900 dark:text-white mb-5 transition-colors duration-300">Sign Up</h2>

          {success ? (
            <div className="text-center py-6">
              <div className="w-12 h-12 mx-auto mb-3 rounded-full bg-[#1a3a2a] flex items-center justify-center">
                <svg width="24" height="24" fill="none" stroke="#4ade80" strokeWidth={2.5} viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                </svg>
              </div>
              <p className="text-[#4ade80] font-medium text-sm">Account created!</p>
              <p className="text-[#666] text-xs mt-1">Redirecting to login...</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {/* Confirm Password */}
              <div>
                <label className="block text-xs font-medium text-gray-500 dark:text-[#888] mb-1.5 transition-colors duration-300">Confirm Password</label>
                <input
                  type={showPass ? 'text' : 'password'}
                  value={confirm}
                  ...
                  autoComplete="new-password"
                  className={[
                    'w-full px-3.5 py-2.5 bg-gray-50 dark:bg-[#1a1a1a] border rounded-xl text-gray-900 dark:text-white text-sm placeholder-gray-400 dark:placeholder-[#444] focus:outline-none transition-all duration-300',
                    confirm && confirm !== password ? 'border-red-500/50 focus:border-red-500' : 'border-gray-200 dark:border-[#2a2a2a] focus:border-blue-500 dark:focus:border-[#4f8ef7]',
                  ].join(' ')}
                />
          ...
          {/* Back to login */}
          {!success && (
            <div className="mt-4 pt-4 border-t border-gray-100 dark:border-[#2a2a2a] text-center transition-colors duration-300">
              <span className="text-xs text-gray-500 dark:text-[#555]">Already have an account? </span>
              <button onClick={onBack} className="text-xs text-blue-600 dark:text-[#4f8ef7] hover:text-blue-700 dark:hover:text-[#7c9ef7] transition-colors font-medium">
                Sign In
              </button>
            </div>
          )}
